...
public class RedisHelper {
    ...
    public void executeAsyncPipeLinedSetMap(Map<String, Object> map, long seconds, TimeUnit timeUnit){
        RBatch batch = redissonClient.createBatch(BatchOptions.defaults());
        map.entrySet().forEach(entry ->{
            if(seconds >= 0){
                batch.getBucket(entry.getKey(), new StringCodec()).setAsync(entry.getValue() instanceof  String
                ? entry.getValue() : JSONUtil.toJsonStr(entry.getValue()), seconds, timeUnit);
            }else{
                batch.getBucket(entry.getKey(), new StringCodec()).setAsync(entry.getValue() instanceof  String
                        ? entry.getValue() : JSONUtil.toJsonStr(entry.getValue()));
            }
        });
        batch.executeAsync();
    }
